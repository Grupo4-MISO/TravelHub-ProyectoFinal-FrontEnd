...
export class ProviderFormComponent {
  ...
  readonly form = this.fb.nonNullable.group({
    ...
    manager: this.fb.nonNullable.group({
  ...
  get managerGroup() {
    return this.form.controls.manager;
  }
  ...
  onSubmit(): void {
    ...
    const manager = this.form.controls.manager.getRawValue();
    const address = this.form.controls.address.getRawValue();

    const payload: CreateProviderPayload = {
      name: company.name.trim(),
      DocumentNumber: company.DocumentNumber.trim(),
      manager: {
        first_name: manager.first_name.trim(),
        last_name: manager.last_name.trim(),
